#!/bin/bash
# Open the usaged web dashboard in the default browser.
...
if ! curl -s -o /dev/null --max-time 3 "http://127.0.0.1:${PORT}/healthz"; then
  echo "usaged is not answering on 127.0.0.1:${PORT}."
  echo "Start it:  launchctl kickstart -k gui/\$(id -u)/com.fcavalcanti.usaged"
  echo "Or check:  launchctl print gui/\$(id -u)/com.fcavalcanti.usaged | grep 'state ='"
  exit 1
fi
...
echo "usaged is up (seq ${SEQ}, data checked ${AGE}s ago) — opening ${URL}"
open "$URL"
