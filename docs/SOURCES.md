...
| OpenCode Zen | **NO BALANCE API EXISTS** (spiked 2026-09-07: ~35 paths across opencode.ai / api.opencode.ai / app.opencode.ai and both bases, every response grepped for the known balance as an oracle — zero matches). The console balance comes from a SolidStart SERVER FUNCTION (`createServerReference("57febdb6…")`, `POST /_server`), authenticated by browser session, NOT by the API key | n/a | same `OPENCODE_API_KEY` — the Zen and Go keys are byte-identical, one credential for both products | n/a | DO NOT build a cookie scraper: undocumented internal RPC, deploy-varying argument format, expiring session, and it would fail silently into a wrong number. Re-probe `/zen/v1/{usage,credits,balance}` to see if a real route has shipped; until then Zen is unshippable and spec task 94 is [BLOCKED] |
