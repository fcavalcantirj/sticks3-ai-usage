...
OpenCode Go was spiked live on 2026-09-07 (see its row and the traps recorded there).
...
| OpenCode Go | `GET https://opencode.ai/zen/go/v1/usage` (header: `Authorization: Bearer <key>`) → `usage.rolling` / `usage.weekly` / `usage.monthly`, each `{status, percent, resetsAt}` | Unofficial, undocumented, unversioned | `OPENCODE_API_KEY` in `.env`, or the macOS Keychain under service `usaged`, account `opencode:go` | n/a (static API key) | The ZEN base `https://opencode.ai/zen/v1` has NO usage route (404) — only the GO base does, and the Go key against the Zen base gives a misleading 500; `api.opencode.ai` is a DIFFERENT host that returns 200 with body `Not Found` for unknown paths, so a status-code-only check there gives a false positive; the console at `opencode.ai/workspace/<id>/go` is a SolidStart app using server functions, NOT a REST API — never scrape it; the API is coarser than that console: three integer percentages and reset times, with NO per-model rows and NO dollar figures, so never render a dollar amount for this provider; `resetsAt` is RFC3339 WITH milliseconds; `POST /chat/completions` returned 500 in the spike, so no Groq-style `x-ratelimit-*` header fallback exists |
| OpenCode Zen | **NO BALANCE API EXISTS.** Spiked 2026-09-07: ~45 paths across opencode.ai / api.opencode.ai / app.opencode.ai, both bases, BOTH auth styles, every response grepped for the known balance as an oracle — zero matches. Decisive test: a route that exists but is mis-authenticated returns **401 JSON AuthError** (e.g. `/zen/go/v1/usage` with `x-api-key`), while a route that does not exist falls through to the marketing SPA and returns **404 HTML** — every balance candidate returns 404-SPA under both auth styles. `docs/zen` documents only inference endpoints; the balance appears solely as prose in Auto-reload | n/a | same `OPENCODE_API_KEY` — the Zen and Go keys are byte-identical, one credential for both products | n/a | Inference is split by model family: GPT -> `POST /zen/v1/responses` (Bearer), Anthropic -> `POST /zen/v1/messages` (`x-api-key`); `/chat/completions` 500s on both bases. A successful `/responses` call returns a top-level `cost` (per-request spend, e.g. 0.00002015) — NOT a balance, and not derivable into one. DO NOT build a cookie scraper against the console's SolidStart server functions. Re-probe `/zen/v1/{usage,credits,balance}`; a 401 instead of a 404-SPA means a real route shipped and spec task 94 unblocks |
