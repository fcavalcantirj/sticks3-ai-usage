...
- THE LEDGER IS \`spec.json\`, 96 tasks, APPEND-ONLY TRUTH: never edit, reorder, delete, renumber or add tasks. The ONLY field you may change is \`passes\`, on the ONE task you did this iteration, after that task's own Verify steps pass. Tasks 1-84 are completed history — never touch them, and never "fix" a task's prose because you disagree with it.
- TASK SELECTION: take the FIRST task with \`passes: false\` whose description does NOT begin with \`[WITHDRAWN\`, \`[DEFERRED\` or \`[BLOCKED\`. Those three are not yours to run — task 77 needs Felipe's hardware UAT, 83 is parked by decision, 94 has no balance API to build against, and 78 and 79 were cancelled with nothing built. Your queue is task 96, the 4-hour provider recommendation. Tasks 85-93 are DONE and approved. Tasks 85-93 are DONE and approved; 95 is Felipe's to close.
