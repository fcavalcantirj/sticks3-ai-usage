...
- THE LEDGER IS \`spec.json\`, 99 tasks, APPEND-ONLY TRUTH: never edit, reorder, delete, renumber or add tasks. The ONLY field you may change is \`passes\`, on the ONE task you did this iteration, after that task's own Verify steps pass. Tasks 1-84 are completed history — never touch them, and never "fix" a task's prose because you disagree with it.
- TASK SELECTION: take the FIRST task with \`passes: false\` whose description does NOT begin with \`[WITHDRAWN\`, \`[DEFERRED\` or \`[BLOCKED\`. Those three are not yours to run — task 77 needs Felipe's hardware UAT, 83 is parked by decision, 94 has no balance API to build against, and 78 and 79 were cancelled with nothing built. Your queue is 96, 97, 98, 99 — the four slices of the 4-hour provider recommendation. Tasks 85-93 are DONE and approved. Tasks 85-93 are DONE and approved; 95 is Felipe's to close.
- TASK 95 IS FELIPE'S TO CLOSE and is now prefixed [BLOCKED] so the loop skips it. Its sweep already ran and passed on 2026-09-08; it was correctly never flipped, because its Verify step names him. Do not flip it, and do not re-run it.
...
- LINE NUMBERS IN TASK STEPS ARE APPROXIMATE AND DRIFT. Every task that edits code shifts the line numbers cited by later tasks — task 85 alone moved cmd/usaged/once.go, serve.go, internal/api/server.go and internal/sched/scheduler.go. ALWAYS locate a symbol by name with grep before editing (\`grep -n 'func buildFetchers' cmd/usaged/once.go\`), and treat a file:line in a step as a hint, never as truth. If the cited line does not contain what the step says it contains, the step is stale — find the real location, do the work, and note the correction in progress.txt. Do NOT edit the ledger to fix a line number.
