...
- THE LEDGER IS \`spec.json\`, 95 tasks, APPEND-ONLY TRUTH: never edit, reorder, delete, renumber or add tasks. The ONLY field you may change is \`passes\`, on the ONE task you did this iteration, after that task's own Verify steps pass. Tasks 1-84 are completed history — never touch them, and never "fix" a task's prose because you disagree with it.
- TASK SELECTION: take the FIRST task with \`passes: false\` whose description does NOT begin with \`[WITHDRAWN\`, \`[DEFERRED\` or \`[BLOCKED\`. Those three are not yours to run — task 77 needs Felipe's hardware UAT, 83 is parked by decision, 94 has no balance API to build against, and 78 and 79 were cancelled with nothing built. Your queue is 85, 86, 87, 88, 89, 90, 91, 92, 93.
- TASK 95 IS THE EXCEPTION AND IT DOES NOT SELF-CERTIFY. When 85-93 are all true, run 95's checks, append a \`UAT:\` line to progress.txt naming what Felipe must confirm, and STOP WITHOUT FLIPPING \`passes\`. Its Verify step names him, and this project's rule is that such a task waits for his word. Do not flip it, do not loop on it — journal, say so, and exit.
